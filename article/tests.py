...
class ArtitclePostViewTests(TestCase):
    ...
    def test_increase_views_but_not_change_updated_field(self):
        ...
        url = reverse('article:article_detail', args=(articel.id, ))
        response = self.client.get(url)

        views_article = ArticlePost.objects.get(id=articel.id)
        self.assertIs(views_article.updated_time - views_article.created_time < timezone.timedelta(seconds=0.1), True)